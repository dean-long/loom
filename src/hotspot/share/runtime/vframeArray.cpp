  ...
  if (bci < 0) {
    ...
    if (ObjectMonitorMode::java() && bci != SynchronizationEntryBCI) {
      if (bci == EpilogueUnlockBci) {
        ...
        assert(bci == PrologueLockBci, "unexpected pseudo-bci %d", bci);
        // monitorenter
      ...
      // FIXME or maybe the counter-overflow.  We could define a different pseudo-bci
      // to distinguish.
      raw_bc = Bytecodes::_monitorenter;

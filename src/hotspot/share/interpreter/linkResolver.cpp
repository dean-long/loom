...
#ifdef ASSERT
  ResourceMark rm(THREAD);
  assert(resolved_method->method_holder()->is_linked(), " %s must be linked", resolved_method->method_holder()->external_name() );
#endif
...
void LinkResolver::resolve_monitorenter(CallInfo& result, TRAPS) {
    methodHandle mh(THREAD, Universe::compiledMonitorEnter());
    result.set_static(mh->method_holder(), mh, CHECK);
}

void LinkResolver::resolve_monitorexit(CallInfo& result, TRAPS) {
    methodHandle mh(THREAD, Universe::compiledMonitorExit());
    result.set_static(mh->method_holder(), mh, CHECK);
}

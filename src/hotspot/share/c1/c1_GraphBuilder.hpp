...
class GraphBuilder {
 ...
 private:
  ...
  BlockBegin*       _start;                      // the start block (first prologue block)
  BlockBegin*       _unwind_handler;             // unwind exception handler for entire outer method
  ...
  void invoke_monitor(Value x, int bci, Bytecodes::Code code, int count, bool needs_cont, Value exception = nullptr);
  void monitorenter(Value x, int bci);
  void monitorexit(Value x, int bci, Value exception = nullptr);
  ...
  void fill_sync_handler(BlockBegin* sync_handler);
 ...
 public:
  ...
  BlockBegin* unwind_handler() const             { return _unwind_handler; }
  void set_unwind_handler(BlockBegin* exc)       { _unwind_handler = exc; }
};

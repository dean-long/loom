...
void GraphBuilder::invoke_monitor(Value x, int bci, Bytecodes::Code code, int count, bool needs_cont) {
  ...
  if (code == Bytecodes::_monitorenter) {
    ...
    apush(append_with_bci(new Constant(objectNull), bci));
  }
...
void GraphBuilder::monitorexit(Value x, int bci) {
  if (ObjectMonitorMode::java()) {
    ...
    bool needs_cont = true;
#endif
    invoke_monitor(x, bci, Bytecodes::_monitorexit, locks, needs_cont);
    CHECK_BAILOUT();
#if 1
    state()->unlock();
...
BlockBegin* GraphBuilder::header_block(BlockBegin* entry, BlockBegin::Flag f, ValueStack* state) {
  ...
  if (do_sync || compilation()->env()->dtrace_method_probes()) {
    ...
    BlockBegin* epilogue = new BlockBegin(SynchronizationEntryBCI);
...
void GraphBuilder::fill_sync_handler(BlockBegin* sync_handler) {
  ...
  assert(lock != nullptr || compilation()->env()->dtrace_method_probes(), "nothing to do");
...
#if 0
  // exit the context of the inlined method
  if (!top_scope) {
XXX FIXME why, this is an exception handler block...
    pop_scope();
...
#if 1
  // throw from SynchronizationEntryBCI, which has no handler
  apush(exception);
  throw_op(bci);
#endif
...
// XXX
  ...
  str.force_bci(SynchronizationEntryBCI);
  scope_data()->set_stream(&str);
  ...
  if (callee->is_synchronized() || compilation()->env()->dtrace_method_probes()) {
    ...
    exc_handler = new BlockBegin(SynchronizationEntryBCI);

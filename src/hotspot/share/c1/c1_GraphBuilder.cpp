  ...
  if (continuation() != nullptr) {
    ...
    if (method()->is_synchronized()) {
      ...
      Value receiver = method()->is_static() ?
        ...
        sync_receiver();
#if 1
assert(sync_receiver() == nullptr || sync_receiver() == state()->lock_at(0),
"local 0 changed?");
#endif
      int bci = SynchronizationEntryBCI;
#if 1
      // XXX JOM would it simplify deopt to have return value already on the stack here?
#endif
...
#if 0
      ipush(append_with_bci(new Constant(new IntConstant(locks)), bci));
      apush(x);
      ipush(lock_stack_pos);
      append fillInLockRecord intrinsic, no call profiling...
#endif
    } else {
...
BlockEnd* GraphBuilder::iterate_bytecodes_for_block(int bci) {
  ...
  while (!bailed_out() && last()->as_BlockEnd() == nullptr &&
    ...
    switch (code) {
      ...
      case Bytecodes::_goto           : _goto(s.cur_bci(), s.get_dest());
#if 1
_fall_through = false;
#endif
break;
      ...
      case Bytecodes::_athrow         : throw_op(s.cur_bci());
#if 1
_fall_through = false;
#endif
break;
      ...
      case Bytecodes::_goto_w         : _goto(s.cur_bci(), s.get_far_dest());
#if 1
_fall_through = false;
#endif
break;
...
BlockBegin* GraphBuilder::setup_start_block(BlockBegin* std_entry) {
  ...
  Base* base = new Base(new_header_block);
...
void GraphBuilder::setup_osr_entry_block() {
  ...
  scope_data()->set_stream(nullptr);
  _start->end()->as_Base()->set_osr_entry(osr_entry);
}
...
#if 1
  bool old_fall_through = _fall_through;
  _fall_through = true;
...
assert(false, "missing pop_scope?");
    }
#endif
  }
#if 1
...
assert(false, "missing pop_scope?");
  ...
  _last = orig_last;
#if 1
  _fall_through = old_fall_through;
...
bool GraphBuilder::try_inline_full(ciMethod* callee, bool holder_known, bool ignore_return, Bytecodes::Code bc, Value receiver, BlockBegin* alt_cont) {
  ...
  assert(_fall_through, "TODO: move to scope_data?");
#endif
...
ScopeData* osd = _scope_data;
  ...
  str.force_bci(SynchronizationEntryBCI, Bytecodes::_monitorenter);
  scope_data()->set_stream(&str);
#endif
...
#if 1
assert(!_skip_block, "sync inlined? should be in invoke_monitor cont block?");
#endif
  }
...
#if 1
// XXX FIXME why not save these for last as out-of-line code?
#endif
